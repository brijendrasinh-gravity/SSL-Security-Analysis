...
function EditUserModal({ show, onHide, userId, roles, onUserUpdated }) {
  ...
  const isChanged = useMemo(() => {
    ...
    return JSON.stringify(current) !== JSON.stringify(original) || !!formik.values.profile_image;
  ...
  const handleClose = () => {
    formik.resetForm();
    setImagePreview(originalData?.profile_image ? `http://localhost:7000${originalData.profile_image}` : null);
    onHide();
  };
