...
function Dashboard() {
  ...
  const [tempLayout, setTempLayout] = useState(null); // stores original layout before edits
  ...
  // HANDLE LAYOUT CHANGE (only mark as unsaved while editing)
  const handleLayoutChange = (newLayout) => {
    if (!isEditMode) return; // ignore changes when not in edit mode
    setLayout(newLayout);
    setHasUnsavedChanges(true);
  };

  // SAVE LAYOUT TO DATABASE (explicit save)
  async function saveLayoutToDB() {
    try {
      ...
      if (response?.data?.success) {
        ...
        setTempLayout(null);
        setIsEditMode(false);
        // optional: show a toast / console
        console.log("Layout saved successfully!");
      } else {
        // handle unsuccessful response if needed
        console.warn("Save responded but success !== true", response?.data);
      }
    } catch (error) {
  ...
  // TOGGLE EDIT MODE (enter/exit with safety)
  const toggleEditMode = () => {
    if (isEditMode) {
      ...
      if (hasUnsavedChanges) {
        ...
        // discard edits and restore original layout
        if (tempLayout) {
          setLayout(JSON.parse(JSON.stringify(tempLayout)));
        } else {
          // if somehow tempLayout missing, reload from server (fallback)
          loadUserLayout();
        }
        setHasUnsavedChanges(false);
      }
      ...
      // entering edit mode: store a deep copy of current layout
      setTempLayout(layout ? JSON.parse(JSON.stringify(layout)) : JSON.parse(JSON.stringify(defaultLayout)));
      setIsEditMode(true);
      setHasUnsavedChanges(false);
    }
  };
  ...
  // Fetch data when global filter changes
  useEffect(() => {
    if (globalFilter.range !== "all" || globalFilter.from || globalFilter.to) {
      fetchAll();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [globalFilter]);
  ...
  const fetchAll = async () => {
    try {
      ...
      // Determine which filter to use for each widget
      const effectiveUserFilter = userFilter.range !== "all" ? userFilter : globalFilter;
      const effectiveVirusFilter = virusFilter.range !== "all" ? virusFilter : globalFilter;
      const effectiveDomainFilter = domainFilter.range !== "all" ? domainFilter : globalFilter;

      const userParams = buildParams(effectiveUserFilter);
      const virusParams = buildParams(effectiveVirusFilter);
      const domainParams = buildParams(effectiveDomainFilter);
  ...
  const FilterUI = ({ filter, setFilter, apply, showReset = true }) => {
    ...
    return (
      <div className="position-relative">
        ...
        {showDropdown && (
          <>
            <div className="filter-backdrop" onClick={() => setShowDropdown(false)} />
    ...
    <div className="chart-card border-0 h-100">
      <div className="chart-header">
        <div className="d-flex justify-content-between align-items-center flex-wrap gap-2">
          ...
          <FilterUI filter={userFilter} setFilter={setUserFilter} apply={fetchAll} showReset={true} />
        </div>
      </div>
    ...
    <div className="chart-card border-0 h-100">
      <div className="chart-header">
        <div className="d-flex justify-content-between align-items-center flex-wrap gap-2">
          ...
          <FilterUI filter={virusFilter} setFilter={setVirusFilter} apply={fetchAll} showReset={true} />
        </div>
      </div>
      <div className="card-body">
        <ResponsiveContainer width="100%" height={250}>
    ...
    <div className="chart-card border-0 h-100">
      <div className="chart-header">
        <div className="d-flex justify-content-between align-items-center flex-wrap gap-2">
          ...
          <FilterUI filter={domainFilter} setFilter={setDomainFilter} apply={fetchAll} showReset={true} />
        </div>
      </div>
      <div className="card-body">
        <ResponsiveContainer width="100%" height={250}>
        ...
        <div className="d-flex justify-content-between align-items-center flex-wrap gap-3">
          <div>
            ...
            <p className="dashboard-subtitle mb-0">
              {isEditMode ? "Edit Mode: Drag widgets to rearrange" : "Overview statistics & trends"}
            </p>
          </div>
          ...
          <div className="d-flex gap-2 align-items-center flex-wrap">
            ...
            {!isEditMode && (
              <>
                <FilterUI filter={globalFilter} setFilter={setGlobalFilter} apply={fetchAll} showReset={false} />
